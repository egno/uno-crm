          ...
          <template v-else>
            <div
              :class="[
                'settings__status',
                {
                  _waiting: props.item.lastStatus.code === 'SMSReserveSum',
                  _success: props.item.lastStatus.code === 'SMSDelivered'
                }
              ]"
            >
            ...
            <div class="sms__time">
              {{ displayRESTDate(props.item.lastStatus.ts) }}
              {{ displayRESTTime(props.item.lastStatus.ts) }}
            </div>
          </template>
        </td>
...
export default {
  ...
  methods: {
    ...
    getData () {
      ...
      BillingApi()
        .get(
          `sms_list/${this.businessId}?order=ts.desc&limit=${
            this.smsPagination.rowsPerPage
          }&offset=${(this.page - 1) * this.smsPagination.rowsPerPage}`
        )
        .then((res) => {
          if (res && res.data) {
