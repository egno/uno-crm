    ...
    <template slot="content">
      ...
      <div :class="['tab-content', { log: activeTab === 1 }]">
        <v-form ref="form">
          <div v-show="activeTab === 0" class="infocard _edit">
            <div class="infocard__content">
              <v-layout align-left column>
                ...
                <div
                  v-for="(event, propName) in businessSettings.notifications
                    .events"
                  :key="propName"
                  class="settings__checkbox-block"
                >
                  <v-switch v-model="event.enabled" :label="event.title" />

                  <div v-if="event.enabled" flat>
                    <div
                      v-if="
                        typeof event.phone !== 'undefined' || event.description
                      "
                    >
                      <div
                        v-if="propName === 'new_visit_client'"
                        class="settings__description"
                      >
                        Шаблон рассылки: <br>
                        Вы записаны на визит
                        к&nbsp;&lt;Должность&nbsp;мастера&gt;
                      ...
                      <div
                        v-else-if="propName === 'new_visit_manager'"
                        class="settings__description"
                      >
                        Шаблон рассылки: <br>
                        Новая онлайн запись к&nbsp;&lt;Мастер&gt;
                        на&nbsp;&lt;Время&nbsp;записи&gt;
                        в&nbsp;&lt;Название&nbsp;филиала&gt;
                      </div>
                      <v-flex
                    ...
                    <div v-if="typeof event.amount !== 'undefined'">
                      <v-flex
                        v-if="typeof event.amount !== 'undefined'"
                        sm6
                        class="amount-input"
                      >
                        <v-text-field
          ...
          <div v-show="activeTab === 2" class="infocard _edit balance">
            <div class="infocard__content">
                ...
                <v-checkbox v-model="oferta_agree" color="blue">
                  <template v-slot:label>
                    <span>Я ознакомился со
                      <a
                        href="https://drive.google.com/open?id=164gkgHbMyDM-esjuOBzdJCQPZtgktIHM"
                        target="_blank"
                      >стоимостью</a>
                      услуг и согласен с
                      <a
                        href="https://docs.google.com/document/d/1Wr31zRe4-mdDyTh8ndn3O1cps0jkQe7aZkYbXTLRw10/"
                        target="_blank"
                      >условиями</a>
                      их предоставления</span>
                  </template>
                  <v-checkbox />
                </v-checkbox>
              </v-layout>
    ...
    saveDisabled () {
      return (
        (this.settings.cancel_visit.enabled &&
          (!this.settings.cancel_visit.phone ||
            this.settings.cancel_visit.phone.length < 10)) ||
        (this.settings.new_visit_manager.enabled &&
          (!this.settings.new_visit_manager.phone ||
            this.settings.new_visit_manager.phone.length < 10))
      )
    },
    settings () {
...
.settings {
  ...
  .balance {
    ...
    &__update {
      ...
      &:hover:after {
        background: url('~assets/images/update_black.png') center/contain
          no-repeat;
        opacity: 1;
      }
      &:after {
