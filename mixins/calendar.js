...
export default {
  ...
  methods: {
    ...
    addMonth (i) {
      const dt = new Date(this.actualDate)
    ...
    dayVisits (dt, employee) {
      ...
      return filtered
        ...
        .map((x) => {
          const ts1 = new Date(x.ts_begin)
          const ts2 = new Date(x.ts_end)
    ...
    fetchData () {
      ...
      Api()
        .get(
          `/visit?salon_id=eq.${this.$route.params.id}&ts_begin=gt.${
            this.selectedWeek[0].dateKey
          }&ts_begin=lt.${formatDate(nextMonday)}`
        )
        .then(({ data }) => {
    ...
    getIrregularDay (dt, employee) {
      ...
      return this.irregularDays.find(
        d => d.date === dt && d.employeeId === employee.id
      )
    },
    goDate (dt) {
